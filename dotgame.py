...
from ai.replay_trainer import ReplayTrainer
from ai.game_recorder import record_game, count_games
try:
...
def main():
    ...
    # Human game replay buffer
    current_game_moves: list = []   # accumulates [player, gx, gy] this game
    ...
    while True:
        ...
        if (not any_overlay
            ...
            state.place(*move)
            current_game_moves.append([cp, move[0], move[1]])
            ...
            if state.game_over:
                ...
                if any(gm.human_controls(p) for p in (1, 2)):
                    record_game(current_game_moves, state.winner_player or 0,
                                dict(state.scores))
                ...
                if event.button in (2, 3) and dragging:
                    ...
                    # ── Training config ───────────────────────────────────────
                        ...
                        if back_r.collidepoint(pos):
                            ...
                            S.MCTS_REAL_ROLLOUT = not S.MCTS_REAL_ROLLOUT
                        elif replay_r and replay_r.collidepoint(pos):
                            # Replay saved human games through current runners
                            from ai.game_recorder import count_games
                            if count_games() > 0:
                                ...
                                try:
                                    rp1 = _make_train_player(1, ai_label,    train_ai_profile_idx)
                                    rp2 = _make_train_player(2, resolved_opp, train_opp_profile_idx)
                                except RuntimeError as exc:
                                    _error_msg    = str(exc)
                                    training_mode = False
                                    picking_mode  = True
                                else:
                                    n_repeats  = _TRAIN_ROUND_PRESETS[train_rnd_idx]
                                    n_games    = count_games()
                                    total_reps = n_games * n_repeats
                                    train_progress[:] = [0, total_reps, {}]
                                    train_cancel_r.clear()

                                    def _on_progress(done, total, wins):
                                        train_progress[:] = [done, total, wins]

                                    def _on_done(wins=None):
                                        train_progress[0] = train_progress[1]

                                    trainer = ReplayTrainer(rp1, rp2, n_repeats,
                    ...
                    # ── Pause menu ────────────────────────────────────────────
                        ...
                        if (on_board and hover
                            ...
                            forbidden_snap = set(state.forbidden_positions)
                            state.place(*hover)
                            current_game_moves.append([human_cp, hover[0], hover[1]])
                            ...
                            if state.game_over:
                                for r in runners.values():
                                    r.on_game_end(state.winner_player,
                                                  final_scores=dict(state.scores))
                                if any(gm.human_controls(p) for p in (1, 2)):
                                    record_game(current_game_moves,
                                                state.winner_player or 0,
                                                dict(state.scores))
                                current_game_moves.clear()
        ...
        if training_mode and not training:
            from ai.game_recorder import count_games as _count_games
